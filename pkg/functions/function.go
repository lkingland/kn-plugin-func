...
type Function struct {
	...
	// This value is used directly to calculate the first part of a function's
	// route, and its service name when deployed to a kubernetes cluster.
	// Therefore it must adhere to the strict rules for DNS labels (subdomains).
	// The relevant RFC for domain names is RFC 1035. Specifically, Section 2.3.1
	// for domain labels (subdomains):
	//   1 may contain letters (a-z, A-Z), digits (0-9), and hyphens (-)
	//   2 may not start or end with a hyphen
	//   3 maximum of 63 characters
	Name string `yaml:"name" jsonschema:"pattern=^(?!-)[a-zA-Z0-9-]{1,63}(?<!-)$`
	...
	Domain string `yaml:"domain" jsonschema:"pattern=^(?:(?:[a-zA-Z0-9](?:[a-zA-Z0-9\-]{0,61}[a-zA-Z0-9])?\.)+[a-zA-Z]{2,6}\.?|[a-zA-Z0-9\-]+\.([a-zA-Z]{2,}\.?))$"`
