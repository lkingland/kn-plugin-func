...
DESCRIPTION
	...
	flag.  Once added, a templates from the repository can be used when creating
	a new Function.
	...
	The Built-in (default) Repository:
	The default built-in repository is not stored on disk, but embedded in the
	binary and can be used without explicitly specifying the name.
	This default repository is always listed first, and is assumed when
	creating a new Function without specifying a repository name prefix.
...
COMMANDS
	...
	With no arguments, this help text is shown.  To confirm repository management
	command to run via an interactive prompt, use the use the --confirm (-c) flag.
	  $ func repository -c
	...
	list
	  List all available repositories, including the installed (default) built-in
...
func (c repositoryClientConfig) prompt() (repositoryClientConfig, error) {
	if !interactiveTerminal() {
		return c, fmt.Errorf("can not prompt a noninteractive terminal")
	}
